...
public class CreateTaskItemCommandHandler(ITaskItemRepository repository, IColumnRepository columnRepository, IBoardRepository boardRepository, ICurrentUser currentUser, IPublishEndpoint publishEndpoint) : IRequestHandler<CreateTaskItemCommand, TaskItemResult>
{
    public async ValueTask<TaskItemResult> Handle(CreateTaskItemCommand request, CancellationToken cancellationToken)
    {
        ...
        var recipientIds = await boardRepository.GetProjectMemberIdsAsync(column.BoardId, cancellationToken);

        await publishEndpoint.Publish(new TaskCreatedEvent(created.Id, created.Title, column.BoardId, created.ColumnId, created.Description, (int)created.Priority, (int)created.Type, created.Order, currentUser.UserId, currentUser.FullName, recipientIds, created.CreatedAt), cancellationToken);
